...
class ROCMAiterMLASparseMetadataBuilder(
    ...
    def __init__(
        ...
        super(AiterMLAMetadataBuilder, self).__init__(kv_cache_spec, layer_names, vllm_config, device)
        ...
        if self._fp8_prefill_enabled:
            ...
            self._init_fp8_prefill_ps_buffers(
                max_num_batched_tokens, max_prefill_qlen, device
            )
    ...
    def _build_decode(
        ...
        attn_metadata.req_id_per_token = req_id_per_token
    ...
    def build(
        ...
        if not self._fp8_prefill_enabled:
            ...
            self.max_query_len = common_attn_metadata.max_query_len
            super_metadata = super(AiterMLAMetadataBuilder, self).build(
            ...
            if metadata.prefill is not None:
                self._build_fp8_prefill_ps_metadata(metadata, common_attn_metadata)
        return metadata
...
class ROCMAiterMLASparseImpl(AiterMLAImpl):
    ...
    def forward_mqa(
        ...
        attn_out = self._forward_mla(
            layer, mla_padded_q, kv_c_and_k_pe_cache, attn_metadata, topk_indices
        )
